...
PARSER.add_argument(
    "--type",
    type=str,
    default="illustrative_copy",
    choices=["copy", "reverse", "illustrative_copy", "illustrative_reverse"],
    help="Type of dataset to generate. One of \"copy\" or \"reverse\" or \"    \"")
PARSER.add_argument(
    "--output_dir",
    type=str,
...
def make_copy_illustrative(num_examples, min_len, max_len):
  """
  ...
  letters_vocab = [chr(x) for x in range(97,123)]
  for _ in range(num_examples):
    turn_length = np.random.choice(np.arange(min_len, max_len + 1))
    source_tokens = np.random.choice(
        list(letters_vocab), size=turn_length, replace=True)
    target_tokens = source_tokens
    yield " ".join(source_tokens), " ".join(target_tokens)

def make_reverse_illustrative(num_examples, min_len, max_len):
  """
  Generates a dataset where the target is equal to the source.
  Sequence lengths are chosen randomly from [min_len, max_len].
...
def main():
  ...
  elif ARGS.type == "illustrative_copy":
    generate_fn = make_copy_illustrative
  elif ARGS.type == "illustrative_reverse":
    generate_fn = make_reverse_illustrative
